...
function handleTranslator() {
    window.gtranslateSettings = {
        ...
        globe_color: "#66aaff",
        wrapper_selector: `.${globalTranslator.value.getAttribute(
            "class"
        )}`,
        flag_size: 16,
        alt_flags: {
...
onMounted(() => {
    ...
    if(document.readyState == "complete" || document.readyState == "interactive") {
        ...
        window.addEventListener("DOMContentLoaded", handleTranslator);
    }
});

onUnmounted(() => {
    window.removeEventListener("DOMContentLoaded", handleTranslator);
});
</script>

<template>
    <div
        class="fixed top-0 left-0 z-10 bg-white w-full py-2 px-12 flex justify-between items-center shadow-sm">
        <div class="flex gap-x-12 items-center">
            <router-link to="/">
                <div aria-label="logo-brand" class="flex items-center">
                    <svg width="99" height="16" viewBox="0 0 99 22" fill="none">
                        <path
            ...
            <ul class="list-none flex gap-x-[3rem] items-center">
                <router-link
                    v-for="({ id, name, dataName, icon }, index) in menuItems"
                    :key="index"
                    :to="{ name: dataName }"
                    >
                    <li class="flex gap-x-2 items-center">
                        <span
                            class="text-[13px] font-[550] text-primaryColor"
                            :data-num="id">
                            {{ name }}
                        </span>
                        <i :class="[icon + ' text-[12px] font-bold']"></i>
                    </li>
                </router-link>
            </ul>
        </div>
        <div class="flex gap-x-8 items-center">
            <router-link to="/user/signin">
                <button class="text-[13px] font-[550] text-primaryColor">
                    Sign In
                </button>
            </router-link>
            <router-link to="/user/signup">
                <button
                    class="text-[13px] font-[550] text-white bg-primaryColor py-[10px] px-6 rounded-lg">
                    Join Kuda
                </button>
            </router-link>
            <div
                class="gtranslate_wrapper"
                :style="translatorStyled"
                ref="globalTranslator"></div>
        </div>
    </div>
</template>
